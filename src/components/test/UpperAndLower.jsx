...
function buildRound() {
  const base = ALL_LETTERS[Math.floor(Math.random() * ALL_LETTERS.length)];
  const showUpper = Math.random() < 0.5;
  const displayed = showUpper ? base.toUpperCase() : base;
  const correct = showUpper ? base : base.toUpperCase();

  const pool = ALL_LETTERS.filter((l) => l !== base);
  const distractors = pool
    .sort(() => Math.random() - 0.5)
    .slice(0, 2)
    .map((l) => (showUpper ? l : l.toUpperCase()));

  const choices = [correct, ...distractors].sort(() => Math.random() - 0.5);
  return { base, displayed, correct, choices };
}

function playLetterName(base) {
  const url = `${LETTER_NAME_BASE}/${base}.mp3`;
  try {
...
export default function UpperAndLower({ onBack, lang = "en", onRoundComplete, hideBackArrow }) {
  ...
  const handleMiddleTap = useCallback(() => {
    playLetterName(round.base);
  }, [round.base]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (selected.letter === round.correct) {
      setShowNext(true);
    } else {
  ...
  }, [selected, round.correct, showNext]);
  ...
  return (
    ...
    >
      ...
      {/* Middle letter — vertically centered, tappable */}
      <div
        style={{
          display: "flex",
          alignItems: "center",
        ...
        <AnimatePresence mode="wait">
          <motion.button
            ...
            style={{
              ...
              background: "white",
              boxShadow:
                "0 12px 40px rgba(78,205,196,0.22), 0 4px 16px rgba(30,58,95,0.10)",
            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  justifyContent: "center",
                  transition:
                    "background 0.18s, border 0.18s, box-shadow 0.18s",
                  WebkitTapHighlightColor: "transparent",
                  flexShrink: 0,
                }}
              >
