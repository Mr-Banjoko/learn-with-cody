/**
 * ShortOLevel9 — Intro Batch C
 * R1-R10: phonics/write pairs for pot, mop, box, fox, jog
 */
...
const ROUNDS = [
  { type: "phonics", word: "pot" },
  { type: "write",   word: "pot" },
  { type: "phonics", word: "mop" },
  { type: "write",   word: "mop" },
  { type: "phonics", word: "box" },
  { type: "write",   word: "box" },
  { type: "phonics", word: "fox" },
  { type: "write",   word: "fox" },
  { type: "phonics", word: "jog" },
  { type: "write",   word: "jog" },
];
...
export default function ShortOLevel9({ onBack, lang = "en" }) {
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {!hintLocked && round.type === "write" && card && <CampaignWriteRound key={`write-${roundIndex}`} card={card} onComplete={advance} onMistake={onMistake} lang={lang} />}
