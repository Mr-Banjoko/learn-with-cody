 ...
 * R1: drawline       — cat / cot / cub (middle letters a/o/u) [cut replaced by cub — no approved asset]
 * R2: identifying    — bed [bed, sad, rid, bud] [bad→sad, bid→rid — no approved asset]
 * R3: missing01      — pig (middle, missing "i", choices a/e/u)
 * R4: drag_v2        — sun (vowel distractor "a")
 * R5: word_to_audio  — cat [cat, cot, hut, kit] [cut→hut — no approved asset]
 * R6: connection     — red
 * R7: writev2        — mug
 * R8: word_match     — pin [pin, pan, pon, pun]
 */
...
const DRAW_LINE_WORDS = [
  { word: "cat", letter: "a", positionType: "middle" },
  { word: "cot", letter: "o", positionType: "middle" },
  { word: "cub", letter: "u", positionType: "middle" },
];
...
export default function CVCChampionLevel78({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("bed"), choices: shuffle([findWord("bed"), findWord("sad"), findWord("rid"), findWord("bud")]) };
  }, [roundIndex]); // eslint-disable-line
  const { photoUrl: sunPhotoUrl, clearPhoto: clearSunPhoto } = useUserPhoto("sun");
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("pig")} forcedMissingPos={1} forcedDistractors={["a", "e", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
            ...
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("cat")} overrideChoices={shuffle([findWord("cat"), findWord("cot"), findWord("hut"), findWord("kit")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("red")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "writev2" && (
              <WriteV2CampaignRound key={`wv2-${roundIndex}`} card={findWord("mug")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "word_match" && (
              <CampaignWordMatchRound key={`wm-${roundIndex}`} card={findWord("pin")} overrideChoices={shuffle([findWord("pin"), findWord("pan"), { word: "pon" }, { word: "pun" }])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
