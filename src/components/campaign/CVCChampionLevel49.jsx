/**
 * CVCChampionLevel49 — "Pack 12 Challenge" (final-mix-level-049)
 * R1: word_to_audio  — mom [mom, mop, mug, man]
 * R2: identifying    — men [men, man, met, mat]
 * R3: rearrange_hard — map
 * R4: dictation      — mug
 * R5: drag_v2        — met (vowel distractor "a")
 * R6: word_match     — man [man, men, min, mun]
 * R7: missing01      — mop (middle, missing "o", choices a/i/u)
 */
...
export default function CVCChampionLevel49({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("men"), choices: shuffle([findWord("men"), findWord("man"), findWord("met"), findWord("mat")]) };
  }, [roundIndex]); // eslint-disable-line

  const rearrangeWordPair = useMemo(() => (round.type === "rearrange_hard" ? [buildWordData("man"), buildWordData("men")] : null), [roundIndex]); // eslint-disable-line
  const { photoUrl: metPhotoUrl, clearPhoto: clearMetPhoto } = useUserPhoto("met");
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("met")} forcedDistractor="a" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={metPhotoUrl} onClearPhoto={clearMetPhoto} />
            )}
            {round.type === "word_match" && (
              <CampaignWordMatchRound key={`wm-${roundIndex}`} card={findWord("man")} overrideChoices={shuffle([findWord("man"), findWord("men"), { word: "min" }, { word: "mun" }])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("mop")} forcedMissingPos={1} forcedDistractors={["a", "i", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
