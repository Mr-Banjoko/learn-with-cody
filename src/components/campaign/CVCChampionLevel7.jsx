/**
 * CVCChampionLevel7 — "Pack 2 Guided Practice" (final-mix-level-007)
 * R1: word_to_audio — pin [pin, pen, pan, pop]
 * R2: missing01     — pen (middle, missing "e", choices e/i/a/o)
 * R3: identifying   — pig [pig, peg, pat]
 * R4: drag_v2       — peg (vowel distractor "i")
 * R5: drawline       — pin/pen/pan (middle vowels i/e/a, shuffled)
 * R6: letter_to_sound_connection — pet
 */
...
export default function CVCChampionLevel7({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("pig"), choices: shuffle([findWord("pig"), findWord("peg"), findWord("pat")]) };
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "word_to_audio" && (
              <CampaignWordMatchRound key={`wta-${roundIndex}`} card={findWord("pin")} overrideChoices={shuffle([findWord("pin"), findWord("pen"), findWord("pan"), findWord("pop")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("pen")} forcedMissingPos={1} forcedDistractors={["i", "a", "o"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("peg")} forcedDistractor="i" onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("pet")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
