...
export default function Level16Complete({ onBack, lang = "en" }) {
  useEffect(() => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, alignItems: "center", justifyContent: "center", padding: "32px 24px", textAlign: "center", fontFamily: "Fredoka, sans-serif" }}>
      <motion.div initial={{ scale: 0.4, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 260, damping: 18, delay: 0.1 }} style={{ fontSize: 90, lineHeight: 1, marginBottom: 16 }}>
        🏆
      </motion.div>
      ...
      <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} style={{ fontSize: 20, color: "#64748B", margin: "0 0 32px", maxWidth: 280 }}>
        {lang === "zh" ? "第 16 关完成！你是明星！⭐" : "Level 16 Complete! You're a star! ⭐"}
      </motion.p>
