/**
 * CVCChampionLevel60 — "Pack 15 Intensive Practice" (final-mix-level-060)
 * R1: dictation      — jab
 * R2: writev2        — jot
 * R3: drag_v2        — jar (vowel distractor "o")
 * R4: word_match     — jet [jet, jat, jot, jut]
 * R5: rearrange_easy — jam
 * R6: missing01      — jig (middle, missing "i", choices a/e/o/u)
 * R7: catch          — jug (missing letter "u", distractors a/e/i/o)
 */
...
export default function CVCChampionLevel60({ onBack, lang = "en" }) {
  ...
  const rearrangeWordPair = useMemo(() => (round.type === "rearrange_easy" ? [buildWordData("jam")] : null), [roundIndex]); // eslint-disable-line
  const { photoUrl: jarPhotoUrl, clearPhoto: clearJarPhoto } = useUserPhoto("jar");
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "dictation" && (
              <DictationCampaignRound key={`dict-${roundIndex}`} card={findWord("jab")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "writev2" && (
              <WriteV2CampaignRound key={`wv2-${roundIndex}`} card={findWord("jot")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("jar")} forcedDistractor="o" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={jarPhotoUrl} onClearPhoto={clearJarPhoto} />
            )}
            {round.type === "word_match" && (
              <CampaignWordMatchRound key={`wm-${roundIndex}`} card={findWord("jet")} overrideChoices={shuffle([findWord("jet"), { word: "jat" }, findWord("jot"), { word: "jut" }])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
