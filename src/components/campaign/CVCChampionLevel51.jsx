/**
 * CVCChampionLevel51 — "Pack 13 Guided Practice" (final-mix-level-051)
 * R1: word_to_audio             — wet [wet, wed, wax, wig]
 * R2: identifying               — wed [wed, wet, wax, vet]
 * R3: missing01                 — wax (middle, missing "a", choices e/o/u)
 * R4: letter_to_sound_connection — wig
 * R5: drag_v2                   — win (vowel distractor "e")
 * R6: draw-a-line               — wet / wax / wig (final letters t/x/g)
 */
...
const DRAW_LINE_WORDS = [
  { word: "wet", letter: "t", positionType: "final" },
  { word: "wax", letter: "x", positionType: "final" },
  { word: "wig", letter: "g", positionType: "final" },
];
...
export default function CVCChampionLevel51({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("wed"), choices: shuffle([findWord("wed"), findWord("wet"), findWord("wax"), findWord("vet")]) };
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("wet")} overrideChoices={shuffle([findWord("wet"), findWord("wed"), findWord("wax"), findWord("wig")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("wax")} forcedMissingPos={1} forcedDistractors={["e", "o", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("wig")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("win")} forcedDistractor="e" onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
