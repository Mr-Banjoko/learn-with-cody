/**
 * CVCChampionLevel4 — "Pack 1 Intensive Practice" (final-mix-level-004)
 * R1: dictation    — cot
 * R2: drag_v2      — bat (vowel distractor "o")
 * R3: missing01    — sit (middle, missing "i", choices i/a/o)
 * R4: word_match   — cat [cat, cot, cit]
 * R5: writev2      — pot
 * R6: rearrange    — mat
 * R7: catch        — hit (distractor letters a/o)
 */
...
export default function CVCChampionLevel4({ onBack, lang = "en" }) {
  ...
  const rearrangeWordPair = useMemo(() => (round.type === "rearrange" ? [buildWordData("mat")] : null), [roundIndex]); // eslint-disable-line
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "dictation" && (
              <DictationCampaignRound key={`dict-${roundIndex}`} card={findWord("cot")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("bat")} forcedDistractor="o" onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "rearrange" && rearrangeWordPair && (
              <PicSliceBoardEasy key={`re-${roundIndex}`} wordPair={rearrangeWordPair} onRoundComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "catch" && (
              <CampaignLetterCatchRound key={`catch-${roundIndex}`} word="hit" missingLetter="i" image={findWord("hit").image} audio={findWord("hit").audio} forcedDistractorLetters={["a", "o"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
