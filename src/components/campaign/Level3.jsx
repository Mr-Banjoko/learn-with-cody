/**
 * Level 3 — Mapping Batch A (connection)
 * R1: connection — cat  [first appearance — audio guide]
 * R2: connection — dad
 * R3: connection — rat
 * R4: connection — hat
 * R5: connection — bat
...
export default function Level3({ onBack, lang = "en" }) {
  ...
  // R1 (index 0): connection first appearance — audio guide + chain word audio
  ...
  const connectionCard = useMemo(() => buildWordData(WORD_ORDER[roundIndex]), [roundIndex]); // eslint-disable-line
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {!hintLocked && <CampaignConnectionRound key={`conn-${roundIndex}`} card={connectionCard} onComplete={advance} lang={lang} onMistake={onMistake} />}
