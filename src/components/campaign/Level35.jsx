/**
 * Level 35 — 10-round Mixed Review (mirrors Level 20 format exactly)
 *
 * Word block: dab, fan, jab, man, nab
 *
 * Round map (same activity pattern as Level 20):
 *  1.  Drag the Letters V2   → dab
 *  2.  Identifying           → dab
 *  3.  Rearrange → Easy      → fan
 *  4.  Identifying           → fan
 *  5.  Missing Sound 0.1     → jab
 *  6.  Identifying           → jab
 *  7.  Drag the Letters V2   → man
 *  8.  Identifying           → man
 *  9.  Missing Sound 0.1     → nab
 * 10.  Draw a Line           → nab, fan, dab
 */
...
import Level35Complete from "./Level35Complete";
...
// ── Missing Letter inline sub-component (same as Level 20) ──────────────────
...
const ROUND_SEQUENCE = [
  { type: "drag",        word: "dab"                      }, // R1
  { type: "identifying", word: "dab"                      }, // R2
  { type: "rearrange",   word: "fan"                      }, // R3
  { type: "identifying", word: "fan"                      }, // R4
  { type: "missing",     word: "jab"                      }, // R5
  { type: "identifying", word: "jab"                      }, // R6
  { type: "drag",        word: "man"                      }, // R7
  { type: "identifying", word: "man"                      }, // R8
  { type: "missing",     word: "nab"                      }, // R9
  { type: "drawline",    words: ["nab", "fan", "dab"]     }, // R10
];
...
export default function Level35({ onBack, lang = "en" }) {
  ...
  const advance = useCallback(() => {
    ...
    if (next >= TOTAL_ROUNDS) { markLevel35Complete(); setDone(true); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "Fredoka, sans-serif", background: "linear-gradient(160deg, #E8FFFE 0%, #FFF9E6 60%, #F5F0FF 100%)", overflow: "hidden" }}>
      <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 4, padding: "calc(env(safe-area-inset-top, 0px) + 8px) 16px 8px", borderBottom: "1.5px solid rgba(0,0,0,0.06)", background: "rgba(255,255,255,0.75)", backdropFilter: "blur(10px)" }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#1E293B" }}>{lang === "zh" ? "第 35 关 — 复习" : "Level 35 — Review"}</p>
        </div>
      ...
      <AnimatePresence mode="wait">
        {done ? (
          <motion.div key="complete" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <Level35Complete onBack={onBack} lang={lang} />
          </motion.div>
        ) : (
