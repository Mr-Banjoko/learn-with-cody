/**
 * Short E — Level 17 — Advanced Recall
 * R1: dictation — gem
 * R2: writev2   — hen
 * R3: dictation — den
 * R4: writev2   — vet
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {roundDef.type === "writev2" && card && <WriteV2CampaignRound key={`wv2-${roundIndex}`} card={card} onComplete={advance} onMistake={onMistake} lang={lang} />}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
