/**
 * Short E — Level 13 — Audio Recall Intro (dictation)
...
export default function ShortELevel13({ onBack, lang = "en" }) {
  ...
  const hintUrl = roundIndex === 0 ? `${GH}/dictation_hint/${lang === "zh" ? "chinese" : "english"}/hint.mp3` : null;
  ...
  const card = findWord(roundDef.word);
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {roundDef.type === "writev2" && card && (
              <WriteV2CampaignRound key={`wv2-${roundIndex}`} card={card} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
