 ...
 * R1: word_to_audio  — red  | red, rad, bed
 * R2: dictation      — ten
...
const DL_DEF = { positionType: "initial", words: [
  { word: "get", targetLetter: "g" },
  { word: "jet", targetLetter: "j" },
  { word: "wet", targetLetter: "w" },
]};
...
const ROUND_SEQUENCE = [
  ...
  { type: "drawline" },
  { type: "writev2",        word: "den" },
  { type: "word_match",     word: "fed" },
];
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {roundDef.type === "dictation" && <DictationCampaignRound key={`dict-${roundIndex}`} card={findWord(roundDef.word)} onComplete={advance} onMistake={onMistake} lang={lang} />}
            {roundDef.type === "rearrange_hard" && rearrangeWordPair && <PicSliceBoard key={`hard-${roundIndex}`} wordPair={rearrangeWordPair} onRoundComplete={advance} lang={lang} onMistake={onMistake} />}
