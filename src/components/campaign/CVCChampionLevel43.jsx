/**
 * CVCChampionLevel43 — "Pack 11 Guided Practice" (final-mix-level-043)
 * R1: word_to_audio — net [net, nut, not, nap]
 * R2: missing01     — nut (middle, missing "u", choices e/a/i)
 * R3: identifying   — not [not, net, nut, nap]
 * R4: drawline      — net / nut / not (final/middle/initial)
 * R5: connection    — bin
 * R6: drag_v2       — ban (vowel distractor "u")
 */
...
const DRAW_LINE_WORDS = [
  { word: "net", letter: "t", positionType: "final" },
  { word: "nut", letter: "u", positionType: "middle" },
  { word: "not", letter: "n", positionType: "initial" },
];
...
export default function CVCChampionLevel43({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("not"), choices: shuffle([findWord("not"), findWord("net"), findWord("nut"), findWord("nap")]) };
  }, [roundIndex]); // eslint-disable-line
  const { photoUrl: banPhotoUrl, clearPhoto: clearBanPhoto } = useUserPhoto("ban");
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("bin")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("ban")} forcedDistractor="u" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={banPhotoUrl} onClearPhoto={clearBanPhoto} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
