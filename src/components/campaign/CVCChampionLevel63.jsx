/**
 * CVCChampionLevel63 — "Pack 16 Guided Practice" (final-mix-level-063)
 * R1: identifying    — rug [rug, rub, rib, rag]
 * R2: missing01      — rub (middle, missing "u", choices i/o/e)
 * R3: word_to_audio  — rib [rib, rub, rob, rag]
 * R4: connection     — rip
 * R5: drawline       — rub / rib / rob (middle letters u/i/o)
 * R6: drag_v2        — rag (vowel distractor "u")
 */
...
const DRAW_LINE_WORDS = [
  { word: "rub", letter: "u", positionType: "middle" },
  { word: "rib", letter: "i", positionType: "middle" },
  { word: "rob", letter: "o", positionType: "middle" },
];
...
export default function CVCChampionLevel63({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("rug"), choices: shuffle([findWord("rug"), findWord("rub"), findWord("rib"), findWord("rag")]) };
  ...
  const { photoUrl: ragPhotoUrl, clearPhoto: clearRagPhoto } = useUserPhoto("rag");
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("rub")} forcedMissingPos={1} forcedDistractors={["i", "o", "e"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("rib")} overrideChoices={shuffle([findWord("rib"), findWord("rub"), findWord("rob"), findWord("rag")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("rip")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("rag")} forcedDistractor="u" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={ragPhotoUrl} onClearPhoto={clearRagPhoto} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
