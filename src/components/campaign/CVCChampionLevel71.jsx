/**
 * CVCChampionLevel71 — "Pack 18 Guided Practice" (final-mix-level-071)
 * R1: identifying    — fed [fed, bed, bet, bit]
 * R2: missing01      — bit (middle, missing "i", choices e/o/u)
 * R3: connection     — bet
 * R4: word_to_audio  — bed [bed, fed, bet, bib]
 * R5: drag_v2        — fan (vowel distractor "i")
 * R6: drawline       — bed / bit / fan (middle letters e/i/a)
 */
...
const DRAW_LINE_WORDS = [
  { word: "bed", letter: "e", positionType: "middle" },
  { word: "bit", letter: "i", positionType: "middle" },
...
export default function CVCChampionLevel71({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("fed"), choices: shuffle([findWord("fed"), findWord("bed"), findWord("bet"), findWord("bit")]) };
  ...
  const { photoUrl: fanPhotoUrl, clearPhoto: clearFanPhoto } = useUserPhoto("fan");
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("bit")} forcedMissingPos={1} forcedDistractors={["e", "o", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("bet")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("bed")} overrideChoices={shuffle([findWord("bed"), findWord("fed"), findWord("bet"), findWord("bib")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("fan")} forcedDistractor="i" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={fanPhotoUrl} onClearPhoto={clearFanPhoto} />
            )}
