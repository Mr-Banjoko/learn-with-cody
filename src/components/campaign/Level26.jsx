/**
 * Level 26 — 10-round sequence (mirrors Level 21 structure):
 * For each of the 5 words, in order:
 *   1. Learn Phonics  (Level1Phonics, no tutorial)
 *   2. Missing Sound 0.1  (MissingSoundGame01, single word)
 *
 * Words (exact order): cab, lad, lab, pal, ban
 *
 * Round map:
 *  1. cab  — phonics
 *  2. cab  — missing sound 0.1
 *  3. lad  — phonics
 *  4. lad  — missing sound 0.1
 *  5. lab  — phonics
 *  6. lab  — missing sound 0.1
 *  7. pal  — phonics
 *  8. pal  — missing sound 0.1
 *  9. ban  — phonics
 * 10. ban  — missing sound 0.1  → marks level complete
 */
...
import Level26Complete from "./Level26Complete";
import { shortAWords } from "../../lib/shortAWords";
import { getLetterSoundUrl, getLetterGain } from "../../lib/letterSounds";
import { playAudio, playAudioSequence } from "../../lib/useAudio";

// ── Inline Missing Sound 0.1 round (same as Level 35's MissingLetterRound) ──
...
function getDistractors(word) {
  ...
  return all.filter((l) => !used.has(l)).sort(() => Math.random() - 0.5).slice(0, 2);
}
...
function MissingLetterRound({ card, onComplete }) {
  ...
  useEffect(() => { return () => { if (sequenceRef.current) { sequenceRef.current(); sequenceRef.current = null; } }; }, []);
  ...
  const handleTouchStart = useCallback((e, option) => {
    ...
    dragStateRef.current = ds; setDragState(ds);
  }, [round.options]);

  const handleTouchMove = useCallback((e) => {
    if (!dragStateRef.current) return;
    e.preventDefault();
    const touch = e.touches[0]; const prev = dragStateRef.current;
    const dx = touch.clientX - prev.startX; const dy = touch.clientY - prev.startY;
    ...
    dragStateRef.current = updated; setDragState(updated);
  }, []);
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-evenly", padding: "10px 20px 14px", minHeight: 0, touchAction: "none", userSelect: "none", position: "relative" }} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
      ...
      <div style={{ background: "rgba(255,255,255,0.55)", borderRadius: 32, padding: "18px 22px", boxShadow: "0 8px 32px rgba(30,58,95,0.10)", border: "2px solid rgba(255,255,255,0.85)", display: "flex", gap: "min(20px, 4vw)", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        {round.letters.map((letter, i) => {
          const isMissing = i === round.missingPos; const isPlacedHere = isMissing && placedOption !== null;
          const isBouncing = bouncingIndex === i; const isWrong = isMissing && feedback === "wrong";
          return (
            <motion.div key={i} ref={isMissing ? dropZoneRef : null} animate={isWrong ? { x: [0,-10,10,-7,7,0] } : isBouncing ? { y: [0,-20,0,-10,0,-4,0] } : {}} transition={{ duration: isWrong ? 0.38 : 0.5 }}
      ...
      <div style={{ display: "flex", gap: "min(16px, 4vw)", justifyContent: "center", flexShrink: 0 }}>
        {round.options.map((option) => {
          const isPlaced = placedOption?.id === option.id; const isDraggingThis = dragState?.id === option.id;
...
const cabCard = findWord("cab");
const ladCard = findWord("lad");
const labCard = findWord("lab");
const palCard = findWord("pal");
const banCard = findWord("ban");

const ROUNDS = [
  { type: "phonics", card: cabCard },
  { type: "missing", card: cabCard },
  { type: "phonics", card: ladCard },
  { type: "missing", card: ladCard },
  { type: "phonics", card: labCard },
  { type: "missing", card: labCard },
  { type: "phonics", card: palCard },
  { type: "missing", card: palCard },
  { type: "phonics", card: banCard },
  { type: "missing", card: banCard },
];

const TOTAL_ROUNDS = ROUNDS.length;

function markLevel26Complete() {
  try {
      ...
      <AnimatePresence mode="wait">
        {done ? (
          <motion.div key="complete" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <Level26Complete onBack={onBack} lang={lang} />
          </motion.div>
        ) : (
