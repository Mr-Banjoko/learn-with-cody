...
const NODE_COLORS = [
  "#FF9F43", "#FFD93D", "#6BCB77", "#4ECDC4", "#4D96FF",
  "#C77DFF", "#FF6B6B", "#FF9F43", "#FFD93D", "#6BCB77",
];
...
export default function ShortOLevels({ onBack, onSelectLevel, lang = "en" }) {
  ...
  const [starMap, setStarMap] = useState(() => {
    const map = {};
    for (let i = 1; i <= TOTAL_LEVELS; i++) map[i] = getBestStars("short-o", i);
    return map;
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "Fredoka, sans-serif", background: "linear-gradient(160deg, #FFF6E8 0%, #FFF9E6 60%, #F5F0FF 100%)", overflow: "hidden" }}>
      <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 4, padding: "calc(env(safe-area-inset-top, 0px) + 8px) 16px 8px", borderBottom: "1.5px solid rgba(0,0,0,0.06)", background: "rgba(255,255,255,0.75)", backdropFilter: "blur(10px)" }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#1E293B" }}>🐙 {lang === "zh" ? "短元音 O" : "Short o"}</p>
          <p style={{ margin: 0, fontSize: 12, color: "#64748B" }}>{lang === "zh" ? "20 关卡冒险" : "20-level adventure"}</p>
        </div>
        <div style={{ background: "#FFF6E8", border: "1.5px solid #FF9F43", borderRadius: 99, padding: "5px 13px", display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
          <span style={{ fontSize: 13 }}>⚡</span>
          <span style={{ color: "#C05B00", fontWeight: 700, fontSize: 13 }}>0 XP</span>
        </div>
      </div>
