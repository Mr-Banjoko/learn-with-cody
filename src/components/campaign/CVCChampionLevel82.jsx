 ...
 * R1: word_to_audio  — sit [sit, sat, jet, jot] [sot→jot, set→jet — no approved asset]
 * R2: dictation      — bag
 * R3: identifying    — bit [bit, bat, bet, hut] [but→hut — no approved asset]
 * R4: writev2        — map
 * R5: drag_v2        — rid (vowel distractor "e")
 * R6: missing01      — fan (middle, missing "a", choices e/o/u)
 * R7: catch          — pin (missing letter "i", distractors a/e/o/u)
 * R8: word_match     — lab [lab, lib, lob, lub]
 */
...
export default function CVCChampionLevel82({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("bit"), choices: shuffle([findWord("bit"), findWord("bat"), findWord("bet"), findWord("hut")]) };
  }, [roundIndex]); // eslint-disable-line
  const { photoUrl: ridPhotoUrl, clearPhoto: clearRidPhoto } = useUserPhoto("rid");
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("sit")} overrideChoices={shuffle([findWord("sit"), findWord("sat"), findWord("jet"), findWord("jot")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "writev2" && (
              <WriteV2CampaignRound key={`wv2-${roundIndex}`} card={findWord("map")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("rid")} forcedDistractor="e" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={ridPhotoUrl} onClearPhoto={clearRidPhoto} />
            )}
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("fan")} forcedMissingPos={1} forcedDistractors={["e", "o", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "catch" && (
              <CampaignLetterCatchRound key={`catch-${roundIndex}`} word="pin" missingLetter="i" image={findWord("pin").image} audio={findWord("pin").audio} forcedDistractorLetters={["a", "e", "o", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "word_match" && (
              <CampaignWordMatchRound key={`wm-${roundIndex}`} card={findWord("lab")} overrideChoices={shuffle([findWord("lab"), { word: "lib" }, { word: "lob" }, { word: "lub" }])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
