/**
 * Short I — Level 9 — Early Spelling Intro
 * R1: writev2    — sit  [audio guide]
 * R2: writev2    — pin
 * R3: drag       — pig
 * R4: identifying — lip
 */
...
const ROUNDS = [
  { type: "writev2",     word: "sit" },
  { type: "writev2",     word: "pin" },
...
export default function ShortILevel9({ onBack, lang = "en" }) {
  ...
  // R1 (index 0): writev2 first appearance in Short I
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "writev2" && card && <WriteV2CampaignRound key={`writev2-${roundIndex}`} card={card} onComplete={advance} onMistake={onMistake} lang={lang} suppressAutoPlay={roundIndex === 0} />}
