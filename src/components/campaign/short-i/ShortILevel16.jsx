/**
 * Short I — Level 16 — Rearrange Hard Intro (Contrast)
 * R1: rearrange_hard — pin + pan (I/A)  [audio guide]
 * R2: rearrange_hard — pin + pen (I/E)
 * R3: rearrange_easy — bin
 * R4: catch          — dig | G (FINAL)
 */
...
// [word1, word2] — first is Short I, second is contrast
const HARD_PAIRS = [
  ["pin", "pan"],  // I vs A
  ["pin", "pen"],  // I vs E
];
...
const ROUNDS = [
  ...
  { type: "rearrange_easy", word: "bin" },
  { type: "catch",          word: "dig", missingLetter: "g" }, // G FINAL
];
...
export default function ShortILevel16({ onBack, lang = "en" }) {
  ...
  // R1 (index 0): rearrange_hard first appearance in Short I
  const hintUrl = roundIndex === 0
    ? `${GH}/rearrange_the_picture_hint/${lang === "zh" ? "chinese" : "english%20"}/hint.mp3`
    : null;
  const r1WordAudio = roundIndex === 0 ? (findI("pin")?.audio || null) : null;
  ...
  const easyWordPair = useMemo(() => round.type === "rearrange_easy" ? [buildWordData(round.word)] : null, [roundIndex]); // eslint-disable-line
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "Fredoka, sans-serif", background: "linear-gradient(160deg, #E8FFFE 0%, #FFF9E6 60%, #F5F0FF 100%)", overflow: "hidden" }}>
      <LevelHeader levelNum={LEVEL_NUM} mistakes={mistakes} onBack={onBack} lang={lang} gameType={round.type === "rearrange_hard" || round.type === "rearrange_easy" ? "rearrange" : round.type} />
      {!done && (
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "rearrange_easy" && easyWordPair && <PicSliceBoardEasy key={`re-${roundIndex}`} wordPair={easyWordPair} onRoundComplete={advance} lang={lang} onMistake={onMistake} />}
