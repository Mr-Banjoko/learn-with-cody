/**
 * Short I — Level 5 — Mapping + Recognition
 * R1: connection  — sit  [audio guide]
 * R2: identifying — pin  [audio guide]
 * R3: connection  — pig
 * R4: identifying — hit
 */
...
const ROUNDS = [
  ...
  { type: "connection",  word: "pig" },
  { type: "identifying", word: "hit" },
];
...
export default function ShortILevel5({ onBack, lang = "en" }) {
  ...
  const hintUrl = useMemo(() => {
    if (roundIndex === 0) return `${GH}/letter_to_sound_hint/${lang === "zh" ? "chinese" : "english%20"}/hint.mp3`;
    if (roundIndex === 1) return `${GH}/identifying_hint/${lang === "zh" ? "chinese" : "english"}/hint.mp3`;
    return null;
  }, [roundIndex, lang]);

  const firstWordAudio = useMemo(() => {
    if (roundIndex === 0) return findWord("sit")?.audio || null;
    if (roundIndex === 1) return findWord("pin")?.audio || null;
    return null;
  }, [roundIndex]);
  ...
  const { locked: hintLocked } = useRoundHintAudio({
    url: hintUrl,
    onHintComplete: (roundIndex === 0 || roundIndex === 1) ? onHintComplete : undefined,
  });
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "identifying" && identifyingRound && (
              <IdentifyingRound key={roundIndex} round={identifyingRound} onComplete={advance} lang={lang} onMistake={onMistake} suppressAutoPlay={roundIndex === 1} />
            )}
