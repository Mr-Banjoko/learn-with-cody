/**
 * CVCChampionLevel23 — "Pack 6 Guided Practice" (final-mix-level-023)
 * R1: identifying    — bug [bug, big, beg, bag]
 * R2: drag_v2        — bog (vowel distractor "u")
 * R3: connection     — beg
 * R4: missing01      — bag (middle, missing "a", choices a/e/i/o)
 * R5: word_to_audio  — big [big, bug, beg, bog]
 * R6: drawline       — bug/big/beg (middle letters u/i/e, shuffled)
 */
...
const DRAW_LINE_WORDS = [
  { word: "bug", letter: "u", positionType: "middle" },
  { word: "big", letter: "i", positionType: "middle" },
  { word: "beg", letter: "e", positionType: "middle" },
];
...
export default function CVCChampionLevel23({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("bug"), choices: shuffle([findWord("bug"), findWord("big"), findWord("beg"), findWord("bag")]) };
  ...
  const { photoUrl: bogPhotoUrl, clearPhoto: clearBogPhoto } = useUserPhoto("bog");
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("bog")} forcedDistractor="u" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={bogPhotoUrl} onClearPhoto={clearBogPhoto} />
            )}
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("beg")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("bag")} forcedMissingPos={1} forcedDistractors={["e", "i", "o"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "word_to_audio" && (
              <CampaignWordMatchRound key={`wta-${roundIndex}`} card={findWord("big")} overrideChoices={shuffle([findWord("big"), findWord("bug"), findWord("beg"), findWord("bog")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
