...
export default function ShortELevel4({ onBack, lang = "en" }) {
  ...
  // R1 (index 0): connection audio guide
  // R3 (index 2): identifying audio guide
  const hintUrl = useMemo(() => {
    if (roundIndex === 0) return `${GH}/letter_to_sound_hint/${lang === "zh" ? "chinese" : "english%20"}/hint.mp3`;
    if (roundIndex === 2) return `${GH}/identifying_hint/${lang === "zh" ? "chinese" : "english"}/hint.mp3`;
    return null;
  }, [roundIndex, lang]);

  const firstWordAudio = useMemo(() => {
    if (roundIndex === 0) return findWord("get")?.audio || null;
    if (roundIndex === 2) return findWord("men")?.audio || null;
    return null;
  }, [roundIndex]);
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "identifying" && identifyingRound && (
              <IdentifyingRound key={roundIndex} round={identifyingRound} onComplete={advance} lang={lang} onMistake={onMistake} suppressAutoPlay={roundIndex === 2} />
            )}
