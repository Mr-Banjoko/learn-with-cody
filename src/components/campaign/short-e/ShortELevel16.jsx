/**
 * Short E — Level 16 — Double Logic + Vowel Contrast
 * R1: rearrange_hard — men + man  [audio guide]
 * R2: rearrange_hard — pen + pan
 * R3: rearrange_hard — bed + bad
 * R4: rearrange_hard — fed + fad
 * R5: rearrange_hard — net + ten
 */
...
export default function ShortELevel16({ onBack, lang = "en" }) {
  ...
  const hintUrl = roundIndex === 0
    ...
    : null;
  const r1WordAudio = roundIndex === 0 ? (findShortE("men")?.audio || null) : null;
  ...
  const wordPair = useMemo(() => buildPair(ROUND_PAIRS[roundIndex]), [roundIndex]); // eslint-disable-line
