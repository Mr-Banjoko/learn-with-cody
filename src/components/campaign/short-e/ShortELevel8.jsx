...
export default function ShortELevel8({ onBack, lang = "en" }) {
  ...
  // R1 (index 0): catch audio guide; R3 (index 2): rearrange_easy audio guide
  const hintUrl = useMemo(() => {
    if (roundIndex === 0) return `${GH}/catch_the_letter_hint/${lang === "zh" ? "chinese" : "english"}/hint.mp3`;
    if (roundIndex === 2) return `${GH}/rearrange_the_picture_hint/${lang === "zh" ? "chinese" : "english%20"}/hint.mp3`;
    return null;
  }, [roundIndex, lang]);

  const firstWordAudio = useMemo(() => {
    if (roundIndex === 0) return findWord("pen")?.audio || null;
    if (roundIndex === 2) return findWord("fed")?.audio || null;
    return null;
  }, [roundIndex]);

  const onHintComplete = useCallback((unlock) => {
    if (!firstWordAudio) { unlock(); return; }
    const audio = new Audio(firstWordAudio);
  ...
  const card = useMemo(() => round.type === "catch" ? findWord(round.word) : null, [roundIndex]); // eslint-disable-line
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "catch" && card && (
              <CampaignLetterCatchRound key={`catch-${roundIndex}`} word={round.word} missingLetter={round.missingLetter} image={card.image} audio={card.audio} onComplete={advance} onMistake={onMistake} lang={lang} paused={hintLocked} skipInitialAudio={roundIndex === 0} />
            )}
            {round.type === "rearrange_easy" && rearrangeWordPair && (
              <PicSliceBoardEasy key={`re-${roundIndex}`} wordPair={rearrangeWordPair} onRoundComplete={advance} lang={lang} onMistake={onMistake} suppressAutoPlay={roundIndex === 2} />
            )}
