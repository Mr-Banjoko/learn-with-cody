/**
 * Short E — Level 13 — Audio Recall Intro
 * R1: dictation — bed  [audio guide]
 * R2: dictation — fed
 * R3: dictation — men
 * R4: writev2   — get
 * R5: writev2   — wet
...
const ROUND_SEQUENCE = [
  ...
  { type: "dictation", word: "men" },
  { type: "writev2",   word: "get" },
...
export default function ShortELevel13({ onBack, lang = "en" }) {
  ...
  const hintUrl = roundIndex === 0
    ? `${GH}/dictation_hint/${lang === "zh" ? "chinese" : "english"}/hint.mp3`
    : null;
  const r1WordAudio = roundIndex === 0 ? (findWord("bed")?.audio || null) : null;
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {roundDef.type === "dictation" && card && <DictationCampaignRound key={`dict-${roundIndex}`} card={card} onComplete={advance} onMistake={onMistake} lang={lang} suppressAutoPlay={roundIndex === 0} />}
