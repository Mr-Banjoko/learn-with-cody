 ...
 * R1: missing01      — net (middle, missing "e", choices a/i/o)
 * R2: connection     — bus
 * R3: identifying    — rag [rag, rug, dig, rob] [rig→dig — no approved asset]
 * R4: word_to_audio  — pot [pot, pat, pet, pit]
 * R5: drag_v2        — hen (vowel distractor "a")
 * R6: word_match     — cub [cub, cab, ceb, cob]
 * R7: dictation      — tip
 * R8: rearrange_hard — wax + fix
 */
...
export default function CVCChampionLevel81({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("rag"), choices: shuffle([findWord("rag"), findWord("rug"), findWord("dig"), findWord("rob")]) };
  }, [roundIndex]); // eslint-disable-line
  const rearrangeWordPair = useMemo(() => (round.type === "rearrange_hard" ? [buildWordData("wax"), buildWordData("fix")] : null), [roundIndex]); // eslint-disable-line
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("net")} forcedMissingPos={1} forcedDistractors={["a", "i", "o"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("pot")} overrideChoices={shuffle([findWord("pot"), findWord("pat"), findWord("pet"), findWord("pit")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
            ...
            {round.type === "word_match" && (
              <CampaignWordMatchRound key={`wm-${roundIndex}`} card={findWord("cub")} overrideChoices={shuffle([findWord("cub"), findWord("cab"), { word: "ceb" }, findWord("cob")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
