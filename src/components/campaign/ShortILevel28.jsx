/**
 * ShortILevel28 — Rearrange Block (all batches)
 * R1: rearrange_easy — big
 * R2: rearrange_easy — sit
 * R3: rearrange_easy — hid
 * R4: rearrange_easy — pig
 * R5: rearrange_easy — win
 */
...
export default function ShortILevel28({ onBack, lang = "en" }) {
  ...
  const wordPair = useMemo(() => [buildShortISliceData(WORD_ORDER[roundIndex])], [roundIndex]); // eslint-disable-line
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "Fredoka, sans-serif", background: "linear-gradient(160deg, #F0F8FF 0%, #FFF9E6 60%, #F5F0FF 100%)", overflow: "hidden" }}>
      <LevelHeader levelNum={LEVEL_NUM} mistakes={mistakes} onBack={onBack} lang={lang} vowelKey="short-i" gameType="rearrange" />
      {!done && (
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <PicSliceBoardEasy key={`re-${roundIndex}`} wordPair={wordPair} onRoundComplete={advance} lang={lang} onMistake={onMistake} />
