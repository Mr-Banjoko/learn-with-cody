/**
 * CVCChampionLevel24 — "Pack 6 Intensive Practice" (final-mix-level-024)
 * R1: dictation      — dog
 * R2: word_match     — bag [bag, beg, big, bog]
 * R3: drag_v2        — dig (vowel distractor "o")
 * R4: writev2        — dad
 * R5: rearrange_easy — beg
 * R6: missing01      — bug (middle, missing "u", choices u/a/e/o)
 * R7: catch          — big (missing letter "i", distractors a/e/o/u)
 */
...
export default function CVCChampionLevel24({ onBack, lang = "en" }) {
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("bug")} forcedMissingPos={1} forcedDistractors={["a", "e", "o"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "catch" && (
              <CampaignLetterCatchRound key={`catch-${roundIndex}`} word="big" missingLetter="i" image={findWord("big").image} audio={findWord("big").audio} forcedDistractorLetters={["a", "e", "o", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
