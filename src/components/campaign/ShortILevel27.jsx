/**
 * ShortILevel27 — Mixed Vowel Review (A + O + I)
 * R1: word_to_audio — big — options: big, bag, bog
 * R2: dictation — cat (Short A)
 * R3: identifying — win
 * R4: word_match — pin — distractors: pan, pon, pun
 * R5: writev2 — fog (Short O)
 */
...
const ROUND_SEQUENCE = [
  { type: "word_to_audio", words: ["big", "bag", "bog"] },
  { type: "dictation",     word: "cat",  source: "a" },
  { type: "identifying",   word: "win",  source: "i" },
  { type: "word_match",    word: "pin",  source: "i", distractors: ["pan", "pon", "pun"] },
  { type: "writev2",       word: "fog",  source: "o" },
];
...
export default function ShortILevel27({ onBack, lang = "en" }) {
  ...
  const card = useMemo(() => {
    ...
    if (def.source === "a") return findA(def.word);
