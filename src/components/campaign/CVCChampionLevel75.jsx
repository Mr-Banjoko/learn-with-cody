/**
 * CVCChampionLevel75 — "Pack 19 Guided Practice" (final-mix-level-075)
 * R1: drag_v2        — tax (vowel distractor "u")
 * R2: identifying    — hem [hem, hen, ham, hug]
 * R3: connection     — hog
 * R4: missing01      — ham (middle, missing "a", choices e/u/i)
 * R5: drawline       — hen / ham / hug (middle letters e/a/u)
 * R6: word_to_audio  — hen [hen, hem, ham, hog]
 */
...
const DRAW_LINE_WORDS = [
  { word: "hen", letter: "e", positionType: "middle" },
  { word: "ham", letter: "a", positionType: "middle" },
  { word: "hug", letter: "u", positionType: "middle" },
];
...
export default function CVCChampionLevel75({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("hem"), choices: shuffle([findWord("hem"), findWord("hen"), findWord("ham"), findWord("hug")]) };
  ...
  const { photoUrl: taxPhotoUrl, clearPhoto: clearTaxPhoto } = useUserPhoto("tax");
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("tax")} forcedDistractor="u" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={taxPhotoUrl} onClearPhoto={clearTaxPhoto} />
            )}
            ...
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("hog")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("ham")} forcedMissingPos={1} forcedDistractors={["e", "u", "i"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("hen")} overrideChoices={shuffle([findWord("hen"), findWord("hem"), findWord("ham"), findWord("hog")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
