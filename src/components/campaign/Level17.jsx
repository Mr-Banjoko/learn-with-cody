/**
 * Level 17 — 5-round Missing Sound 0.1
 *
 * Round order:
 *  1. tag
 *  2. jar
...
import Level17Complete from "./Level17Complete";
...
const WORD_ORDER = ["tag", "jar", "gas", "tap", "bag"];
      ...
      <AnimatePresence mode="wait">
        {done ? (
          <motion.div key="complete" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <Level17Complete onBack={onBack} lang={lang} />
          </motion.div>
        ) : (
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {card && <MissingLetterRound key={`missing-${roundIndex}`} card={card} onComplete={advance} />}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
