 ...
 * R1: identifying    — wet [wet, sit, hot, wax] [wit→sit, wot→hot — no approved asset]
 * R2: word_to_audio  — fog [fog, fig, fed, fun]
 * R3: connection     — nut
 * R4: drawline       — wet / win / wax (middle letters e/i/a)
 * R5: drag_v2        — jog (vowel distractor "u")
 * R6: missing01      — pup (middle, missing "u", choices a/e/o)
 * R7: dictation      — leg
 * R8: word_match     — cot [cot, cat, cit, cut]
 */
...
const DRAW_LINE_WORDS = [
  { word: "wet", letter: "e", positionType: "middle" },
  { word: "win", letter: "i", positionType: "middle" },
  { word: "wax", letter: "a", positionType: "middle" },
];
...
export default function CVCChampionLevel83({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("wet"), choices: shuffle([findWord("wet"), findWord("sit"), findWord("hot"), findWord("wax")]) };
  ...
  const { photoUrl: jogPhotoUrl, clearPhoto: clearJogPhoto } = useUserPhoto("jog");
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("fog")} overrideChoices={shuffle([findWord("fog"), findWord("fig"), findWord("fed"), findWord("fun")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("nut")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("jog")} forcedDistractor="u" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={jogPhotoUrl} onClearPhoto={clearJogPhoto} />
            )}
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("pup")} forcedMissingPos={1} forcedDistractors={["a", "e", "o"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "dictation" && (
              <DictationCampaignRound key={`dict-${roundIndex}`} card={findWord("leg")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
