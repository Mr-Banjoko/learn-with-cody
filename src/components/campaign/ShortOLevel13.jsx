/**
 * ShortOLevel13 — Batch D Practice (rearrange_easy)
 * R1: rearrange_easy — sob
 * R2: rearrange_easy — cod
 * R3: rearrange_easy — cop
 * R4: rearrange_easy — cot
 * R5: rearrange_easy — bog
 */
...
const WORD_ORDER = ["sob", "cod", "cop", "cot", "bog"];
...
export default function ShortOLevel13({ onBack, lang = "en" }) {
  ...
  const wordPair = useMemo(() => [buildShortOSliceData(WORD_ORDER[roundIndex])], [roundIndex]); // eslint-disable-line
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "Fredoka, sans-serif", background: "linear-gradient(160deg, #FFF6E8 0%, #FFF9E6 60%, #F5F0FF 100%)", overflow: "hidden" }}>
      <LevelHeader levelNum={LEVEL_NUM} mistakes={mistakes} onBack={onBack} lang={lang} vowelKey="short-o" gameType="rearrange" />
      {!done && (
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {!hintLocked && <PicSliceBoardEasy key={`re-${roundIndex}`} wordPair={wordPair} onRoundComplete={advance} lang={lang} onMistake={onMistake} />}
