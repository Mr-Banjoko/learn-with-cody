/**
 * CVCChampionLevel40 — "Pack 10 Intensive Practice" (final-mix-level-040)
 * R1: dictation      — sip
 * R2: writev2        — sub
 * R3: word_match     — sat [sat, set, sit, sot]
 * R4: rearrange_easy — sob
 * R5: drag_v2        — sun (vowel distractor "a")
 * R6: missing01      — sum (middle, missing "u", choices a/i/o/e)
 * R7: catch          — sad (missing letter "a", distractors e/i/o/u)
 */
...
export default function CVCChampionLevel40({ onBack, lang = "en" }) {
  ...
  const rearrangeWordPair = useMemo(() => (round.type === "rearrange_easy" ? [buildWordData("sob")] : null), [roundIndex]); // eslint-disable-line
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "dictation" && (
              <DictationCampaignRound key={`dict-${roundIndex}`} card={findWord("sip")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "writev2" && (
              <WriteV2CampaignRound key={`wv2-${roundIndex}`} card={findWord("sub")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "word_match" && (
              <CampaignWordMatchRound key={`wm-${roundIndex}`} card={findWord("sat")} overrideChoices={shuffle([findWord("sat"), { word: "set" }, findWord("sit"), { word: "sot" }])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
