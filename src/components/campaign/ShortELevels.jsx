...
const NODE_COLORS = [
  "#FF6B6B", "#FF9F43", "#FFD93D", "#6BCB77", "#4ECDC4", "#4D96FF", "#C77DFF",
];
...
export default function ShortELevels({ onBack, onSelectLevel, lang = "en" }) {
  ...
  useEffect(() => {
    ...
    const lastCompleted = Object.keys(map).map(Number).filter((l) => map[l] > 0).reduce((max, l) => Math.max(max, l), 0);
    ...
    requestAnimationFrame(() => {
      el.scrollTo({ top: Math.max(0, nodeTopPx - vh / 2 + NODE_SPACING / 2), behavior: "smooth" });
    });
  }, []);
