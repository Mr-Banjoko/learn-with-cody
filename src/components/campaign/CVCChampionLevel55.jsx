/**
 * CVCChampionLevel55 — "Pack 14 Guided Practice" (final-mix-level-055)
 * R1: identifying               — fox [fox, fix, six, box]
 * R2: word_to_audio             — fix [fix, fox, six, gas]
 * R3: draw-a-line               — fox / fix / gas (middle letters o/i/a)
 * R4: missing01                 — box (middle, missing "o", choices i/e/u)
 * R5: letter_to_sound_connection — bus
 * R6: drag_v2                   — bug (vowel distractor "o")
 */
...
const DRAW_LINE_WORDS = [
  { word: "fox", letter: "o", positionType: "middle" },
  { word: "fix", letter: "i", positionType: "middle" },
  { word: "gas", letter: "a", positionType: "middle" },
];
...
export default function CVCChampionLevel55({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("fox"), choices: shuffle([findWord("fox"), findWord("fix"), findWord("six"), findWord("box")]) };
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("fix")} overrideChoices={shuffle([findWord("fix"), findWord("fox"), findWord("six"), findWord("gas")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("box")} forcedMissingPos={1} forcedDistractors={["i", "e", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("bus")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("bug")} forcedDistractor="o" onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
