/**
 * CVCChampionLevel32 — "Pack 8 Intensive Practice" (final-mix-level-032)
 * R1: dictation      — lad
 * R2: drag_v2        — lip (vowel distractor "e")
 * R3: writev2        — pal
 * R4: word_match     — leg [leg, lag, log, lug]
 * R5: rearrange_easy — lab
 * R6: catch          — log (missing letter "o", distractors a/e/i/u)
 * R7: missing01      — pug (middle, missing "u", choices a/e/i/o)
 */
...
export default function CVCChampionLevel32({ onBack, lang = "en" }) {
  ...
  const rearrangeWordPair = useMemo(() => (round.type === "rearrange_easy" ? [buildWordData("lab")] : null), [roundIndex]); // eslint-disable-line
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "dictation" && (
              <DictationCampaignRound key={`dict-${roundIndex}`} card={findWord("lad")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
            ...
            {round.type === "writev2" && (
              <WriteV2CampaignRound key={`wv2-${roundIndex}`} card={findWord("pal")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "word_match" && (
              <CampaignWordMatchRound key={`wm-${roundIndex}`} card={findWord("leg")} overrideChoices={shuffle([findWord("leg"), { word: "lag" }, findWord("log"), { word: "lug" }])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
