/**
 * Level 33 — 5-round Missing Sound 0.1
 *
 * Round order:
 *  1. man
 *  2. nab
...
import Level33Complete from "./Level33Complete";
import { shortAWords } from "../../lib/shortAWords";
import { getLetterSoundUrl, getLetterGain } from "../../lib/letterSounds";
import { playAudio, playAudioSequence } from "../../lib/useAudio";

// ── Inline Missing Letter round ───────────────────────────────────────────────
...
function MissingLetterRound({ card, onComplete }) {
  ...
  const handleTouchMove = useCallback((e) => {
    if (!dragStateRef.current) return; e.preventDefault();
...
export default function Level33({ onBack, lang = "en" }) {
  ...
  const advance = useCallback(() => {
    ...
    if (next >= TOTAL_ROUNDS) { markLevel33Complete(); setDone(true); }
    else setRoundIndex(next);
  }, [roundIndex]);

  const progressPct = (roundIndex / TOTAL_ROUNDS) * 100;
  const wordName = WORD_ORDER[roundIndex];
  const currentCard = useMemo(
    () => shortAWords.find((w) => w.word === wordName),
      ...
      <AnimatePresence mode="wait">
        {done ? (
          <motion.div key="complete" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <Level33Complete onBack={onBack} lang={lang} />
          </motion.div>
        ) : (
