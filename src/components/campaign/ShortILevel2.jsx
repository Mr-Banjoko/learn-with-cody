...
export default function ShortILevel2({ onBack, lang = "en" }) {
  ...
  const connectionCard = useMemo(() => (round.type === "connection" || round.type === "letter_to_sound") ? buildShortISliceData(round.word) : null, [roundIndex]); // eslint-disable-line
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {(round.type === "connection" || round.type === "letter_to_sound") && connectionCard && <CampaignConnectionRound key={`conn-${roundIndex}`} card={connectionCard} onComplete={advance} lang={lang} onMistake={onMistake} />}
