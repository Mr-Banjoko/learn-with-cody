/**
 * CVCChampionLevel33 — "Pack 8 Challenge" (final-mix-level-033)
 * R1: rearrange_hard — pug + log
 * R2: identifying     — lip [lip, lid, leg, log]
 * R3: word_to_audio   — pal [pal, lab, lad, pug]
 * R4: dictation       — leg
 * R5: drag_v2         — lad (vowel distractor "e")
 * R6: word_match      — lid [lid, lad, led, lod, lud]
 * R7: missing01       — log (middle, missing "o", choices a/e/i/u)
 */
...
export default function CVCChampionLevel33({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("lip"), choices: shuffle([findWord("lip"), findWord("lid"), findWord("leg"), findWord("log")]) };
  }, [roundIndex]); // eslint-disable-line

  const rearrangeWordPair = useMemo(() => (round.type === "rearrange_hard" ? [buildWordData("pug"), buildWordData("log")] : null), [roundIndex]); // eslint-disable-line
  const { photoUrl: ladPhotoUrl, clearPhoto: clearLadPhoto } = useUserPhoto("lad");
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "word_to_audio" && (
              <CampaignWordMatchRound key={`wta-${roundIndex}`} card={findWord("pal")} overrideChoices={shuffle([findWord("pal"), findWord("lab"), findWord("lad"), findWord("pug")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
