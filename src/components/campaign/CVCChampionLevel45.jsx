/**
 * CVCChampionLevel45 — "Pack 11 Challenge" (final-mix-level-045)
 * R1: identifying    — bun [bun, bin, ban, nut]
 * R2: word_to_audio  — nab [nab, nap, net, not]
 * R3: dictation      — bin
 * R4: rearrange_hard — nut
 * R5: word_match     — ban [ban, ben, bin, bun]
 * R6: drag_v2        — net (vowel distractor "i")
 * R7: missing01      — nap (middle, missing "a", choices e/o/u)
 */
...
export default function CVCChampionLevel45({ onBack, lang = "en" }) {
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("nab")} overrideChoices={shuffle([findWord("nab"), findWord("nap"), findWord("net"), findWord("not")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "dictation" && (
              <DictationCampaignRound key={`dict-${roundIndex}`} card={findWord("bin")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            ...
            {round.type === "word_match" && (
              <CampaignWordMatchRound key={`wm-${roundIndex}`} card={findWord("ban")} overrideChoices={shuffle([findWord("ban"), { word: "ben" }, findWord("bin"), findWord("bun")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("net")} forcedDistractor="i" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={netPhotoUrl} onClearPhoto={clearNetPhoto} />
            )}
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("nap")} forcedMissingPos={1} forcedDistractors={["e", "o", "u"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
