/**
 * ShortELevel8 — Draw-a-Line Batch B (-et family)
 * R1: drawline — {bet, get, jet} — final: t,t,t → use initial: b,g,j
 * R2: drawline — {met, net, pet} — initial: m,n,p
 * R3: drawline — {vet, wet, get} — initial: v,w,g
 * R4: writev2 — vet
 * R5: writev2 — wet
 */
...
const DRAW_DEFS = [
  { words: [{ word: "bet", letter: "b" }, { word: "get", letter: "g" }, { word: "jet", letter: "j" }], shuffleOrder: [2, 0, 1] },
  { words: [{ word: "met", letter: "m" }, { word: "net", letter: "n" }, { word: "pet", letter: "p" }], shuffleOrder: [2, 0, 1] },
  { words: [{ word: "vet", letter: "v" }, { word: "wet", letter: "w" }, { word: "get", letter: "g" }], shuffleOrder: [1, 2, 0] },
];

const ROUND_SEQUENCE = [
  { type: "drawline", defIdx: 0 },
  { type: "drawline", defIdx: 1 },
  { type: "drawline", defIdx: 2 },
  { type: "writev2", word: "vet" },
  { type: "writev2", word: "wet" },
];
...
export default function ShortELevel8({ onBack, lang = "en" }) {
  ...
  const drawLineRound = useMemo(() => roundDef.type === "drawline" ? buildDrawLineRound(DRAW_DEFS[roundDef.defIdx]) : null, [roundIndex]); // eslint-disable-line
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {!hintLocked && roundDef.type === "drawline" && drawLineRound && <DrawLineBoard key={`dl-${roundIndex}`} round={drawLineRound} onRoundComplete={advance} lang={lang} onMistake={onMistake} />}
