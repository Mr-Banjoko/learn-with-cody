...
const NODE_COLORS = ["#FF6B6B","#FF9F43","#FFD93D","#6BCB77","#4ECDC4","#4D96FF","#C77DFF"];
...
function LevelNode({ num, color, onTap, isMilestone, stars, isFinal, lang = "en" }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      <motion.div whileTap={{ scale: 0.85 }} onClick={() => onTap(num)} style={{ width: size, height: size, borderRadius: "50%", background: isFinal ? "linear-gradient(145deg, #FFD700, #FFA500)" : `linear-gradient(145deg, ${color} 0%, ${color}CC 100%)`, border: isFinal ? "4px solid white" : "3px solid white", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: isFinal ? "0 7px 0 #cc8800, 0 12px 28px rgba(255,165,0,0.55)" : `0 6px 0 ${color}99, 0 10px 22px ${color}44`, WebkitTapHighlightColor: "transparent", position: "relative", flexShrink: 0 }}>
        {isFinal ? (
          <span style={{ fontSize: 36, pointerEvents: "none", lineHeight: 1 }}>🏆</span>
        ) : (
          <>
            {isMilestone && <span style={{ position: "absolute", top: -18, fontSize: 18, pointerEvents: "none", filter: "drop-shadow(0 1px 3px rgba(0,0,0,0.15))" }}>⭐</span>}
...
export default function ShortALevels({ onBack, onSelectLevel, lang = "en" }) {
  ...
  const [starMap, setStarMap] = useState(() => {
    const map = {};
    for (let i = 1; i <= TOTAL_LEVELS; i++) map[i] = getBestStars("short-a", i);
    return map;
  });
  ...
  useEffect(() => {
    ...
    const focusLevel = Math.max(Math.min(lastCompleted + 1, TOTAL_LEVELS), 1);
    const nodeTopPx = TOP_OFFSET + (focusLevel - 1) * NODE_SPACING;
    const scrollContainer = scrollRef.current;
    if (!scrollContainer) return;
    const targetScrollTop = nodeTopPx - scrollContainer.clientHeight / 2 + NODE_SPACING / 2;
    requestAnimationFrame(() => scrollContainer.scrollTo({ top: Math.max(0, targetScrollTop), behavior: "smooth" }));
  }, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "Fredoka, sans-serif", background: "linear-gradient(160deg, #E8FFFE 0%, #FFF9E6 60%, #F5F0FF 100%)", overflow: "hidden" }}>
      <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 4, padding: "calc(env(safe-area-inset-top, 0px) + 8px) 16px 8px", borderBottom: "1.5px solid rgba(0,0,0,0.06)", background: "rgba(255,255,255,0.75)", backdropFilter: "blur(10px)" }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#1E293B" }}>🍎 {lang === "zh" ? "短元音 A" : "Short a"}</p>
          <p style={{ margin: 0, fontSize: 12, color: "#64748B" }}>{lang === "zh" ? "40 关卡冒险" : "40-level adventure"}</p>
        </div>
        ...
        <div style={{ position: "relative", width: "100%", height: TOP_OFFSET + TOTAL_LEVELS * NODE_SPACING + 80 }}>
          {levels.map((lvl, idx) => (
            <div key={lvl} style={{ position: "absolute", top: TOP_OFFSET + idx * NODE_SPACING, left: `${getLeftPct(idx)}%`, transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center" }}>
              <LevelNode num={lvl} color={nodeColor(lvl)} isMilestone={lvl % 10 === 0} isFinal={lvl === TOTAL_LEVELS} onTap={onSelectLevel || (() => {})} stars={starMap[lvl] ?? 0} lang={lang} />
            </div>
          ))}
        </div>
      </div>
