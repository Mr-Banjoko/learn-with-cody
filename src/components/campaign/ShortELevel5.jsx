/**
 * Short E — Level 5 — Reinforcement
 * R1: missing01  — bet | T (FINAL)
 * R2: drag       — met
 * R3: connection — bed
 * R4: identifying — red
 * R5: missing01  — get | E (MEDIAL)
 */
...
const ROUND_SEQUENCE = [
  { type: "missing01",  word: "bet",  missingPos: 2 }, // T FINAL
  { type: "drag",       word: "met" },
  { type: "connection", word: "bed" },
  { type: "identifying",word: "red" },
  { type: "missing01",  word: "get",  missingPos: 1 }, // E MEDIAL
];
...
export default function ShortELevel5({ onBack, lang = "en" }) {
        ...
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {roundDef.type === "missing01" && (
              <CampaignMissingSound01Round key={`missing-${roundIndex}`} card={findWord(roundDef.word)} forcedMissingPos={roundDef.missingPos} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {roundDef.type === "drag" && (
              <Level1DragV2 key={`drag-${roundIndex}`} card={findWord(roundDef.word)} onComplete={advance} lang={lang} onMistake={onMistake} />
            )}
            {roundDef.type === "connection" && connectionCard && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={connectionCard} onComplete={advance} lang={lang} onMistake={onMistake} />
            )}
