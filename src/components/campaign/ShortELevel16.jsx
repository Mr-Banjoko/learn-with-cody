/**
 * Short E — Level 16 — Double Logic + Vowel Contrast (rearrange_hard)
 * R1: men + man  [audio guide]
 * R2: pen + pan
 * R3: bed + bad
 * R4: fed + fad
 * R5: net + ten  (anagram logic)
 */
...
export default function ShortELevel16({ onBack, lang = "en" }) {
  ...
  const hintUrl = roundIndex === 0 ? `${GH}/rearrange_the_picture_hint/${lang === "zh" ? "chinese" : "english%20"}/hint.mp3` : null;
  ...
  const wordPair = useMemo(() => ROUND_WORDS[roundIndex].map(buildWordData), [roundIndex]); // eslint-disable-line
