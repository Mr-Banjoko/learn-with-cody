/**
 * CVCChampionLevel59 — "Pack 15 Guided Practice" (final-mix-level-059)
 * R1: drag_v2        — jet (vowel distractor "i")
 * R2: identifying    — jig [jig, jog, jug, jet]
 * R3: connection     — jug
 * R4: missing01      — jam (middle, missing "a", choices o/u/e)
 * R5: word_to_audio  — jog [jog, jig, jug, jam]
 * R6: drawline       — jog / jig / jug (middle letters o/i/u)
 */
...
const DRAW_LINE_WORDS = [
  { word: "jog", letter: "o", positionType: "middle" },
  { word: "jig", letter: "i", positionType: "middle" },
  { word: "jug", letter: "u", positionType: "middle" },
];
...
export default function CVCChampionLevel59({ onBack, lang = "en" }) {
  ...
  const identifyingRound = useMemo(() => {
    ...
    return { target: findWord("jig"), choices: shuffle([findWord("jig"), findWord("jog"), findWord("jug"), findWord("jet")]) };
  }, [roundIndex]); // eslint-disable-line
  const { photoUrl: jetPhotoUrl, clearPhoto: clearJetPhoto } = useUserPhoto("jet");
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div key={`round-${roundIndex}`} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.22 }} style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {round.type === "drag_v2" && (
              <Level1DragV2 key={`dv2-${roundIndex}`} card={findWord("jet")} forcedDistractor="i" onComplete={advance} onMistake={onMistake} lang={lang} userPhotoUrl={jetPhotoUrl} onClearPhoto={clearJetPhoto} />
            )}
            ...
            {round.type === "connection" && (
              <CampaignConnectionRound key={`conn-${roundIndex}`} card={findWord("jug")} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "missing01" && (
              <CampaignMissingSound01Round key={`ms-${roundIndex}`} card={findWord("jam")} forcedMissingPos={1} forcedDistractors={["o", "u", "e"]} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
            {round.type === "word_to_audio" && (
              <CampaignWordToAudioRound key={`wta-${roundIndex}`} card={findWord("jog")} overrideChoices={shuffle([findWord("jog"), findWord("jig"), findWord("jug"), findWord("jam")])} onComplete={advance} onMistake={onMistake} lang={lang} />
            )}
