...
import DragTheLettersGameV2 from "./DragTheLettersGameV2";
...
export default function DragTheLettersV2({ onBack, lang = "en" }) {
  const [selected, setSelected] = useState(null);

  if (selected) {
    const group = VOWEL_GROUPS.find((g) => g.id === selected);
    return (
      <DragTheLettersGameV2
        words={group.words}
        title={group.label}
        color={group.color}
  ...
  return (
    <div className="min-h-full pb-32" style={{ background: "linear-gradient(160deg, #E8FFFE 0%, #FFF9E6 60%, #F5F0FF 100%)", fontFamily: "Fredoka, sans-serif" }}>
      ...
      <div style={{ background: "#A8D0E6", borderBottomLeftRadius: 28, borderBottomRightRadius: 28, padding: "10px 20px 16px", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "#1E3A5F" }}>{lang === "zh" ? "拖拽字母 V2 ✋" : "Drag the Letters V2 ✋"}</h1>
