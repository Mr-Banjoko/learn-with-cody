...
function buildRoundState(rawRound) {
  const top = rawRound.map((entry, i) => ({ id: i, word: entry.word, audio: entry.audio, targetLetter: entry.targetLetter }));
  let bottom;
  let attempts = 0;
  do {
    bottom = shuffle(rawRound.map((entry, i) => ({ id: i, letter: entry.targetLetter, soundUrl: getLetterSoundUrl(entry.targetLetter), gain: getLetterGain(entry.targetLetter) })));
    attempts++;
...
export default function DrawALineGame({ rounds, onComplete, lang = "en" }) {
  ...
  const advanceRound = useCallback(() => {
    ...
    if (next >= rounds.length) {
      ...
      setBouncingBottom(null); setBouncingTop(null); setRevealedLetters({}); setLocked(false);
    }
  ...
  return (
    <div ref={containerRef} style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", padding: "10px 14px 12px", overflow: "hidden", paddingTop: "15%", paddingBottom: "20%", pointerEvents: locked ? "none" : "auto" }}>
          ...
          return (
            <div key={item.id} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              <motion.button
                ...
                animate={isBouncing ? { y: [0, -14, 0, -8, 0, -4, 0], scale: [1, 1.08, 1, 1.04, 1] } : { y: 0, scale: 1 }}
                transition={isBouncing ? { duration: 0.6, ease: "easeOut" } : {}}
                onPointerDown={(e) => { e.stopPropagation(); if (!locked) playAudio(item.audio); }}
                style={{ width: "100%", padding: "11px 4px", borderRadius: 18, background: isWrong ? "#FEE2E2" : matched ? mc + "25" : "white", border: `2.5px solid ${isWrong ? "#FF6B6B" : matched ? mc : "#E2E8F0"}`, boxShadow: isWrong ? "0 0 0 3px #FF6B6B55, 0 4px 18px rgba(255,107,107,0.35)" : matched ? `0 4px 18px ${mc}40` : "0 3px 12px rgba(30,58,95,0.09)", fontSize: "clamp(20px, 5.5vw, 28px)", fontWeight: 700, fontFamily: "Fredoka, sans-serif", color: "#1E293B", cursor: "pointer", textAlign: "center", WebkitTapHighlightColor: "transparent", transition: "background 0.18s, border 0.18s, box-shadow 0.18s", letterSpacing: 1 }}
              >
                {item.word}
              </motion.button>
              <div onPointerDown={(e) => { e.stopPropagation(); handleTopBox(item.id); }} style={{ padding: 8, margin: -8, cursor: matched ? "default" : "pointer", touchAction: "manipulation", WebkitTapHighlightColor: "transparent", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div ref={(el) => (topBoxRefs.current[item.id] = el)} style={{ width: 28, height: 28, borderRadius: 7, border: "2.5px solid", transition: "all 0.16s", pointerEvents: "none", ...getTopBoxStyle(item.id) }} />
              </div>
            </div>
          );
        })}
          ...
          return (
            <div key={item.id} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              <div onPointerDown={(e) => { e.stopPropagation(); handleBottomBox(item.id); }} style={{ padding: 8, margin: -8, cursor: matched ? "default" : "pointer", touchAction: "manipulation", WebkitTapHighlightColor: "transparent", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div ref={(el) => (bottomBoxRefs.current[item.id] = el)} style={{ width: 28, height: 28, borderRadius: 7, border: "2.5px solid", transition: "all 0.16s", pointerEvents: "none", ...getBottomBoxStyle(item.id) }} />
              </div>
              <motion.div animate={isBouncing ? { y: [0, -16, 0, -9, 0, -4, 0], scale: [1, 1.15, 1, 1.07, 1] } : { y: 0, scale: 1 }} transition={isBouncing ? { duration: 0.55, ease: "easeOut" } : {}} style={{ width: "100%" }}>
                <AnimatePresence mode="wait">
                  {revealedLetter ? (
                    <motion.div key="letter" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: "spring", stiffness: 300, damping: 18 }} style={{ width: "100%", padding: "10px 4px", borderRadius: 20, background: mc ? mc + "25" : "white", border: `2.5px solid ${mc || "#E2E8F0"}`, boxShadow: mc ? `0 4px 18px ${mc}40` : "0 3px 12px rgba(30,58,95,0.09)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "clamp(26px, 7vw, 36px)", fontWeight: 700, fontFamily: "Fredoka, sans-serif", color: mc || "#1E293B" }}>
                      {revealedLetter}
                    </motion.div>
                  ) : (
                    <motion.button key="speaker" initial={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.6 }} transition={{ duration: 0.15 }} whileTap={locked ? {} : { scale: 0.90 }} onPointerDown={(e) => { e.stopPropagation(); if (!locked && item.soundUrl) playAudio(item.soundUrl, item.gain); }} style={{ width: "100%", padding: "13px 4px", borderRadius: 20, background: isWrong ? "#FEE2E2" : "white", border: `2.5px solid ${isWrong ? "#FF6B6B" : "#E2E8F0"}`, boxShadow: isWrong ? "0 0 0 3px #FF6B6B55, 0 4px 18px rgba(255,107,107,0.35)" : "0 3px 12px rgba(30,58,95,0.09)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", WebkitTapHighlightColor: "transparent", transition: "background 0.18s, border 0.18s, box-shadow 0.18s" }}>
