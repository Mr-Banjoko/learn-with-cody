/**
 * Placement round — Upper & Lower.
 * An uppercase letter is shown (tappable to hear its name); the learner taps the
 * matching lowercase letter. Fixed round (from props), campaign-round interface.
 */
...
export default function PlacementUpperLower({ round, onComplete, onMistake, lang = "en" }) {
  const { base, displayed, correct, choices } = round;
  const [selected, setSelected] = useState(null);
  const [showNext, setShowNext] = useState(false);
  const [wrongShake, setWrongShake] = useState(false);
  const shakeTimeout = useRef(null);

  const handleChoiceTap = useCallback((letter) => {
    if (showNext) return;
    setSelected(letter);
  ...
  }, [selected, correct, showNext, onMistake]);
      ...
      <div style={{ flexShrink: 0, display: "flex", justifyContent: "center", gap: 20, paddingLeft: 24, paddingRight: 24, marginBottom: 28 }}>
        <motion.div animate={wrongShake ? { x: [0, -10, 10, -8, 8, 0] } : { x: 0 }} transition={{ duration: 0.45 }} style={{ display: "flex", gap: 20 }}>
          {choices.map((letter, idx) => {
            ...
            return (
              <motion.button
                key={`${displayed}-${letter}-${idx}`}
                onClick={() => handleChoiceTap(letter)}
                whileTap={{ scale: 0.91 }}
                style={{
