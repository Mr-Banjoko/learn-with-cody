      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pt-6">
        {total === 0 ? (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center gap-4 mt-16">
            ...
            <p style={{ fontSize: 20, fontWeight: 600, color: "#4A90C4", textAlign: "center" }}>No flashcards saved yet!</p>
            <p style={{ fontSize: 15, color: "#7BACC8", textAlign: "center", maxWidth: 260 }}>Go to Learn Phonics, take a photo, and save your first flashcard here.</p>
          </motion.div>
        ) : (
          <div className="flex flex-col items-center gap-6 w-full" style={{ maxWidth: 360 }}>
            <p style={{ fontSize: 16, fontWeight: 600, color: "#4A90C4" }}>{index + 1} of {total}</p>
            <AnimatePresence mode="wait">
              <motion.div key={card.id} initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.94 }} transition={{ duration: 0.2 }} style={{ width: "100%", borderRadius: 24, overflow: "hidden", boxShadow: "0 12px 40px rgba(30,58,95,0.15)" }}>
                <img src={card.snapshot} alt={card.word} style={{ width: "100%", display: "block" }} />
              </motion.div>
            </AnimatePresence>
            ...
            {!confirmDelete ? (
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <button onClick={() => setConfirmDelete(false)} style={{ padding: "10px 24px", borderRadius: 999, background: "#C5DCF0", color: "#1E3A5F", border: "none", fontSize: 16, fontWeight: 600, cursor: "pointer", fontFamily: "Fredoka, sans-serif" }}>Cancel</button>
                <button onClick={handleDelete} style={{ padding: "10px 24px", borderRadius: 999, background: "#EF4444", color: "white", border: "none", fontSize: 16, fontWeight: 600, cursor: "pointer", fontFamily: "Fredoka, sans-serif" }}>Yes, Delete</button>
              </div>
            )}
          </div>
        )}
